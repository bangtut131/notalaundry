...
import { Shirt, Mail, Lock, Eye, EyeOff, Store, User } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ storeName, name, email, password })
      });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8 animate-in fade-in zoom-in-95 duration-500">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Daftar Baru</h1>
          <p className="text-muted-foreground mt-2">Mulai kelola laundry Anda secara digital</p>
        </div>
        ...
        <div className="rounded-2xl border border-border/50 bg-card p-6 shadow-lg space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Nama Laundry / Toko</label>
              <div className="relative">
                <Store className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-4 h-4" />
                <input type="text" value={storeName} onChange={e => setStoreName(e.target.value)} required
                  ...
                  placeholder="Laundry Bersih Cemerlang" />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium">Nama Anda (Pemilik)</label>
              <div className="relative">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? 'Mendaftarkan...' : 'Daftar & Mulai Gratis'}
            </button>
          </form>

          <p className="text-xs text-center text-muted-foreground">
            Paket Starter gratis selama 1 bulan pertama.
          </p>
        </div>
